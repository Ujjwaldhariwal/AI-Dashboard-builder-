...
test.describe('client dashboard data and NLP editing', () => {
  ...
  test('keeps raw data and governed edit controls in the client runtime', () => {
    ...
    expect(gridSource).toContain('mode="presentation_only"')
    expect(gridSource).toContain('dashboardChartPresentationToWidgetStyle')
    expect(gridSource).toContain('tooltipFields={state.resolved.tooltipFields}')
    expect(gridSource).toContain('data-density={density}')
    expect(gridSource).toContain('density={chart.presentation.density}')
