...
type ChartHealthState = 'healthy' | 'stale' | 'blocked'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (status !== 'all') chartQuery = chartQuery.eq('status', status)

    const { data: chartRows, error: chartError } = await chartQuery
    if (chartError) return NextResponse.json({ audit: null, error: chartError.message }, { status: 500 })
    ...
    const { data: datasetRows, error: datasetError } = datasetIds.length > 0
      ? await auth.supabase.from('semantic_datasets').select('id, status, selection').in('id', datasetIds)
      : { data: [], error: null }

    if (datasetError) return NextResponse.json({ audit: null, error: datasetError.message }, { status: 500 })
    ...
    const [fieldsResult, metricsResult] = await Promise.all([
      fieldIds.size > 0
        ? auth.supabase.from('business_fields').select('*').in('id', Array.from(fieldIds))
        : Promise.resolve({ data: [], error: null }),
      metricIds.size > 0
        ? auth.supabase.from('business_metrics').select('*').in('id', Array.from(metricIds))
        : Promise.resolve({ data: [], error: null }),
    ])

    if (fieldsResult.error) return NextResponse.json({ audit: null, error: fieldsResult.error.message }, { status: 500 })
    if (metricsResult.error) return NextResponse.json({ audit: null, error: metricsResult.error.message }, { status: 500 })
    ...
    const items = charts.map(chart => {
      ...
      const validation = dataset
        ...
        : { state: 'invalid' as const, issues: [{ severity: 'error' as const, code: 'missing_dataset', message: 'Dataset not found.' }] }
