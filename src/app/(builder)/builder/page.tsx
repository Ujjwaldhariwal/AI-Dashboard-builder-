...
export default function BuilderPage() {
    ...
    return (
      <div className="p-6">
        <BuilderHeader
          ...
          exporting={exporting} aiPanelOpen={aiPanelOpen}
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      {/* ── AI Side Panel ─────────────────────────────────────────────────── */}
      <div
        className={`flex-shrink-0 border-l bg-card transition-all duration-300 ease-in-out overflow-hidden ${
      ...
      {/* ── AI toggle tab (always visible when panel closed) ─────────────── */}
      {!aiPanelOpen && (
        <button
          onClick={() => setAiPanelOpen(true)}
          className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex items-center gap-1.5 bg-gradient-to-b from-blue-600 to-purple-600 text-white text-xs font-medium px-2 py-4 rounded-l-xl shadow-lg hover:from-blue-700 hover:to-purple-700 transition-all writing-mode-vertical"
...
function BuilderHeader({
  ...
  aiPanelOpen, onAddWidget, onMagicOpen, onExport, onToggleAI,
}: {
  currentDash:  any
  widgets:      any[]
  endpoints:    any[]
