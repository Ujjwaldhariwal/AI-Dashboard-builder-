-- Repair drifted ops tables so publish health recording and alert delivery
...
alter table dashboard_health_runs
  ...
  add column if not exists healthy_slots integer not null default 0,
  add column if not exists stale_slots integer not null default 0,
  add column if not exists blocked_slots integer not null default 0,
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'dashboard_health_runs'
      and column_name = 'status'
  ) then
    update dashboard_health_runs
    set health_state = coalesce(health_state, nullif(status, ''), 'blocked')
    where health_state is null;
  else
    update dashboard_health_runs
    set health_state = coalesce(health_state, 'blocked')
    where health_state is null;
  end if;
end $$;
...
alter table dashboard_health_runs
  drop constraint if exists dashboard_health_runs_status_check,
  drop constraint if exists dashboard_health_runs_health_state_check;

alter table dashboard_health_runs
  add constraint dashboard_health_runs_health_state_check
  check (health_state in ('healthy', 'stale', 'blocked'));
